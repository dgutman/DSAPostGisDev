...
tissueSeg_model.load_state_dict(
    torch.load("tissueModelbest.pt", map_location=torch.device("cpu"))
)
model.eval()
...
def reshape_with_pad(img, size, pad=(255, 255, 255)):
    ...
    if w > h:
        img = cv.copyMakeBorder(img, 0, w - h, 0, 0, cv.BORDER_CONSTANT, None, pad)
    else:
        img = cv.copyMakeBorder(img, 0, 0, 0, h - w, cv.BORDER_CONSTANT, None, pad)
...
def selected(selected):
    if selected:
        ...
        pred = cv.resize(
            pred, (orig_shape[0], orig_shape[1]), interpolation=cv.INTER_NEAREST
        )
...
def selected(selected):
    if selected:
        ...
        layout = go.Layout(title=selected[0]["name"], showlegend=False)
        ...
        return dbc.Container(
            [
                dbc.Row([dbc.Col(html.Div(selected[0]["name"]))]),
                dbc.Row([dbc.Col(image_fig), dbc.Col(dcc.Graph(figure=fig))]),
            ]
        )
